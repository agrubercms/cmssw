...
private:
  ...
  static pat::PackedCandidate buildPackedCandidate(const reco::PFCandidate* cand,
    ...
    if (track) {
      ...
      packed.setTrackHighPurity(cand->trackRef().isNonnull() &&
...
hltParticleTransformerAK4TagInfoProducer::hltParticleTransformerAK4TagInfoProducer(const edm::ParameterSet& iConfig)
    : jet_radius_(iConfig.getParameter<double>("jet_radius")),
      ...
      puppi_value_map_token_(
          consumes<edm::ValueMap<float>>(iConfig.getParameter<edm::InputTag>("puppi_value_map"))),
...
void hltParticleTransformerAK4TagInfoProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const bool use_puppi_value_map = puppi_value_map.isValid();
    ...
    if (jet.pt() < min_jet_pt_ || std::abs(jet.eta()) > max_jet_eta_) {
      ...
      // Fill secondary vertex features
      {
        ...
        for (const auto& sv : svs_sorted) {
          ...
          svfeat.jet_sv_costhetasvpv = (flip_ ? -1.f : 1.f) * cos_sv_pv;
      ...
      for (unsigned int i = 0; i < jet.numberOfDaughters(); ++i) {
        ...
        if (use_puppi_value_map) {
          edm::Ref<edm::View<reco::Candidate>> candRef = getPersistentCandidate(cand, tracks);
          if (candRef.isNonnull()) {
            constexpr double min_puppi_wgt = -1.0;
            if ((*puppi_value_map)[candRef] < min_puppi_wgt)
              continue;
          }
        }
      ...
      std::sort(pfCandidates.begin(), pfCandidates.end(),
                [](const reco::PFCandidate* a, const reco::PFCandidate* b) { return a->pt() > b->pt(); });
      ...
      for (const auto* cand : pfCandidates) {
        ...
        pat::PackedCandidate packed_candidate =
            buildPackedCandidate(cand, track, pvAssocQual, pv_ass, *PVRefProd);
        math::XYZPoint pv_ass_pos = pv_ass->position();
