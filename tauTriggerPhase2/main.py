...
#events = Events('/eos/user/a/agruber/samples/HLT_Upgrade_L1filter/VBF_HTauTau_14_1_0_pre7_TriggerOnly/Phase2_L1P2GT_HLT_90.root')
events = Events(files)
#events = Events('Phase2_L1P2GT_HLT_noL1path.root')
    ...
    for category in histogram_categories:
        if "pt" in category:
            histograms_with_cuts[path][category] = ROOT.TH1D(
                f"{category}_{path}_withCuts", f"{category.replace('_', ' ').title()} for {path} with pt>35, |eta|<2.1",
                histogram_config["pt"]["bins"], histogram_config["pt"]["min"], histogram_config["pt"]["max"]
            )
        elif "eta" in category:
            histograms_with_cuts[path][category] = ROOT.TH1D(
...
for event_nr, event in enumerate(events):
    ...
    if len(hadronic_taus) == 2:
        ...
        continue # Skip events with fewer than 2 hadronic taus - doesn't make too much difference in final plots
    ...
    for path, filter_name in trigger_paths:
        ...
        for key in filter_keys:
            ...
            passes_pt_obj = obj.pt() > 35
            passes_eta_obj = abs(obj.eta()) < 2.1
            if passes_eta_obj:
        ...
        if leading_tau:
            ...
            # Process Subleading Tau Only If Leading Tau Is Matched
    ...
    if event_counter > 2:
        print(f"Event {event_nr} has {event_counter} hadronic taus passing the trigger.")
